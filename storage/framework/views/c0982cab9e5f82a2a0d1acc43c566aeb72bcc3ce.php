			  ...
			  <div class="box-body">
				...
				<div class="direct-chat-messages" id="message">
					...
					<div class="direct-chat-msg " v-bind:class="[activeClass]"  v-for="message in messages" :key="message.id">
				        ...
				        <img class="direct-chat-img" src="<?php echo e(url('')); ?>/images/user2-160x160.jpg" alt="message user image">
			  	 ...
			  	 <ul class="products-list product-list-in-box" id="userStatus">
	                <!-- <li class="item">
	                  <div class="product-img">
	                    <img src="<?php echo e(url('')); ?>/images/user2-160x160.jpg" alt="Product Image">
	                  </div>
...
<!-- <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/io/2.0.4/socket.io.js"></script> -->
    ...
    <script >
    	...
    	function htmlChat(data){
        	...
        	text += '<img class="direct-chat-img" src="<?php echo e(url('')); ?>/images/user2-160x160.jpg" alt="message user image">'
		...
		var param = {
		        	...
		        	"classEdit" : '<?php echo e(Auth::user()->id); ?>',
		        	"UID" : '<?php echo e(Auth::user()->id); ?>',
		}
		// var socket = io.connect('http://serveragscom-as.cloud.revoluz.io:49707',{query:"name=<?php echo e(Auth::user()->name); ?>"});
		// var socket = io.connect('http://serveragscom-as.cloud.revoluz.io',{query:"name=<?php echo e(Auth::user()->name); ?>"});
		// var socket = io.connect('http://192.168.66.51:3000',{query:"name=<?php echo e(Auth::user()->name); ?>"});
		...
		new Vue({
		    ...
		    data: {
		        ...
		        isRight:false,
		        init:true,
		        activeClass:'customRight<?php echo e(Auth::user()->id); ?>',
		        usernames: [],
		        username: '<?php echo e(Auth::user()->name); ?>',
		        classEdit: '<?php echo e(Auth::user()->id); ?>',
		    },
		    mounted: function() {
		        yuuAPP.socket.on('chat.message', function(data) {
		        	...
		        	console.log('<?php echo e(Auth::user()->id); ?>');
		        	var uID = '<?php echo e(Auth::user()->id); ?>';
		            if (data.message != "") {
		        	...
		        	console.log(data);
					    ...
					    if (data.action == "connect") {
		        			if($("#item" + itemId).length == 0) {
						    	...
						    	spanChat = "";
				            	if (data.name != "<?php echo e(Auth::user()->name); ?>") {
				            		spanChat = '<span class="label label-info pull-right">Chat</span>';
					            }
						    	text += '<li class="item" id="item'+itemId+'">';
			                  	text += '<div class="product-img">'
		       ...
		       /* yuuAPP.socket.on('disconnect',function(){
		        	console.log('disconnect : <?php echo e(Auth::user()->id); ?>');
